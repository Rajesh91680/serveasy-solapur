...
// //       if (found.role === "admin") { setError("Admin access not allowed."); return; }
...
// //       // ✅ Only show address popup if there's a pending booking
// //       const pending = sessionStorage.getItem("pendingProviderSelection");
// //       if (pending) {
// //         setLoggedInUser(found);
// //         setShowAddrPopup(true);
...
import { Link, useNavigate } from "react-router-dom";
import { Home, X, User, LogOut, ChevronDown } from "lucide-react";
import { getCurrentUser, setCurrentUser } from "../services/store";

import Login from "./pages/user/auth/Login";
import Signup from "./pages/user/auth/Signup";
import AddressSelectPopup from "./pages/user/auth/AddressSelectPopup";
  ...
  const handleAddressConfirm = (addressLine) => {
    ...
    if (pending) {
      try {
        ...
        if (window.__onAddressConfirmed) window.__onAddressConfirmed(addressLine);
      } catch {
        sessionStorage.removeItem("pendingProviderSelection");
      }
    } else {
          ...
          <div className="bg-white p-8 rounded-xl w-full max-w-[440px] relative max-h-[90vh] overflow-y-auto">
            ...
            {loading && (
              <div className="absolute inset-0 bg-white/70 flex items-center justify-center rounded-xl">
                <p className="text-orange-500 font-semibold">Please wait...</p>
              </div>
            )}
          </div>
        </div>
