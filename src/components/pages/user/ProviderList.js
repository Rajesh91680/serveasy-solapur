      ...
      {reviewProvider && (
        <ReviewModal provider={reviewProvider}
          ...
          onBook={() => {
            ...
            const bookingData = { service:state?.service||"General Service", address:state?.address||"", description:desc, date:state?.date||new Date().toLocaleDateString(), time:state?.time||"ASAP", providers:[p.name], bookingId:`BK${Date.now().toString().slice(-6)}` };
            if (!currentUser) { sessionStorage.setItem("pendingProviderSelection", JSON.stringify(bookingData)); if (window.__navbarOpenModal) window.__navbarOpenModal("login"); return; }
            addBooking({ userId:currentUser.id, userName:currentUser.name, userEmail:currentUser.email, ...bookingData, status:"requested" });
