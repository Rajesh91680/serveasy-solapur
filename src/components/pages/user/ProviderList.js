// // ============================================================
// // FILE: src/components/pages/user/ProviderList.js
// // ============================================================
...
// // ── Description Popup ──────────────────────────────────────────────────────
// function DescPopup({ provider, value, onChange, onSendAll, selectedProviders, service, onClose }) {
//   const ref = useRef();
//   useEffect(() => {
//     const h = (e) => e.key === "Escape" && onClose();
//     window.addEventListener("keydown", h);
...
//           <button onClick={handleSendAll} style={{ flex:2, padding:"11px", background:"#1A3C6E", color:"white", border:"none", borderRadius:"9px", fontWeight:700, fontSize:"13px", cursor:"pointer" }}>💬 Save & Send WhatsApp to All</button>
//         </div>
//       </div>
//     </div>
//   );
...
//     if (navigator.share) { navigator.share({ title:"Service Provider Details", text }); }
...
//           <button onClick={handleShare} style={{ flex:1, padding:"10px", borderRadius:"10px", border:"none", background:"#1A3C6E", color:"white", fontWeight:700, fontSize:"12px", cursor:"pointer" }}>
//             {copied ? "✓ Copied!" : "🔗 Share Provider"}
//           </button>
//           <button onClick={handleBook} disabled={booking}
//             style={{ flex:2, padding:"10px", borderRadius:"10px", border:"none", background:booking?"#86EFAC":"#22C55E", color:"white", fontWeight:700, fontSize:"13px", cursor:booking?"not-allowed":"pointer", boxShadow:"0 3px 10px rgba(34,197,94,0.3)" }}>
...
//   const handleSelect = (val) => { onRespond(providerId, val); setOpen(false); };
//   return (
...
//           <div onClick={() => handleSelect("yes")} style={{ padding:"10px 12px", cursor:"pointer", fontSize:"12px", fontWeight:600, color:"#166534", background:isYes?"#DCFCE7":"white", borderBottom:"1px solid #F3F4F6" }}
//             onMouseOver={(e)=>e.currentTarget.style.background="#DCFCE7"} onMouseOut={(e)=>e.currentTarget.style.background=isYes?"#DCFCE7":"white"}>✅ Yes — Available</div>
//           <div onClick={() => handleSelect("no")} style={{ padding:"10px 12px", cursor:"pointer", fontSize:"12px", fontWeight:600, color:"#991B1B", background:isNo?"#FEE2E2":"white" }}
//             onMouseOver={(e)=>e.currentTarget.style.background="#FEE2E2"} onMouseOut={(e)=>e.currentTarget.style.background=isNo?"#FEE2E2":"white"}>❌ No — Unavailable</div>
//           {(isYes||isNo) && (
//             <div onClick={() => handleSelect(null)} style={{ padding:"6px 10px", cursor:"pointer", fontSize:"10px", color:"#9CA3AF", borderTop:"1px solid #F3F4F6", textAlign:"center" }}
//               onMouseOver={(e)=>e.currentTarget.style.background="#F9FAFB"} onMouseOut={(e)=>e.currentTarget.style.background="white"}>✕ Clear response</div>
//           )}
//         </div>
//       )}
//     </div>
...
//         return (
//           <div key={s.label} style={{ border:`1px solid ${done?"#22C55E":active?s.color:"#E5E7EB"}`, borderRadius:"5px", background:done?"#F0FDF4":active?s.bg:"#FAFAFA", padding:"3px 5px", opacity:locked?0.45:1 }}>
...
//           <div style={{ fontSize:"12px", color:"#6B7280", marginBottom:"10px" }}>WhatsApp will be sent to all <strong>{providers.length} providers</strong></div>
//           <div style={{ display:"flex", gap:"5px", flexWrap:"wrap" }}>
//             {providers.map(p => (<span key={p.id} style={{ padding:"2px 8px", background:"#EEF4FF", color:"#1A3C6E", borderRadius:"20px", fontSize:"10px", fontWeight:600, border:"1px solid #BFDBFE" }}>{p.name}</span>))}
//           </div>
//         </div>
//         <div style={{ padding:"16px 20px" }}>
//           <textarea value={desc} onChange={e => setDesc(e.target.value)} placeholder="e.g. AC not cooling, pipe is leaking, need urgent repair..." autoFocus
...
//           <button onClick={handleSend} style={{ flex:2, padding:"12px", borderRadius:"10px", border:"none", background:"#1A3C6E", color:"white", fontWeight:800, fontSize:"14px", cursor:"pointer" }}>💬 Send WhatsApp to All →</button>
//         </div>
//       </div>
//     </div>
//   );
...
//       s[p.id] = { waSent:false, availability:null, confirmed:false, reviewed:false, description:state?.description||"" };
//     });
//     return s;
//   });
...
//   const handleProceed = () => {
//     const confirmed  = shownProviders.filter(p => ps[p.id]?.confirmed);
//     if (!confirmed.length) return;
//     const chosenNames = confirmed.map(p => p.name);
//     const finalDesc   = confirmed.map(p => ps[p.id]?.description?.trim()).filter(Boolean).join(" | ") || state?.description || "";
//     navigate("/booking-request", { state: { ...state, providers:chosenNames, description:finalDesc } });
//   };

//   // ══════════════════════════════════════════════════════════════════════════
//   // ✅ handleBookNow — saves ALL confirmed providers to Django DB
//   //
//   // Sends: { provider_names: ["Ravi Kumar", "Suresh Patil", ...], service, date }
//   // Django loops through provider_names and saves ONE row per provider.
//   //
//   // addBooking() from store.js only saves to localStorage — no API call.
//   // ══════════════════════════════════════════════════════════════════════════
...
//     if (!p) { bookingInProgress.current = false; return; }

//     // ✅ Collect ALL confirmed provider names (not just the one in modal)
//     const confirmedProviders = shownProviders.filter(pr => ps[pr.id]?.confirmed);
//     const providerNames = confirmedProviders
//       .map(pr => pr.name?.trim())
//       .filter(name => name && name.length > 0);

//     // Fallback: if somehow none confirmed, use the provider open in modal
//     if (providerNames.length === 0) providerNames.push(p.name.trim());

//     console.log("🚀 Booking — sending provider_names to Django:", providerNames);

//     try {
//       // ✅ ONE POST request — Django saves one DB row per name in the array
//       const res = await fetch(`${API_BASE}/api/service-requests/`, {
//         method:  "POST",
//         headers: { "Content-Type": "application/json" },
//         body: JSON.stringify({
//           provider_names: providerNames,                          // ✅ array
...
//       if (res.ok) {
//         console.log(`✅ ${responseData.bookings?.length || 1} booking(s) saved in DB`);

//         // Close the review modal and mark provider as reviewed
//         handleCloseReview(p.id);
...
//           bookingId:   responseData.id, // first DB row id returned by Django
//         };

//         // If not logged in — save pending booking and open login modal
//         if (!currentUser) {
...
//         // ✅ Save to localStorage only (addBooking does NOT call any API)
//         addBooking({
...
//       } else {
//         console.error("❌ Django API error:", responseData);
//         alert("❌ Booking failed: " + (responseData.error || "Unknown error") + "\n\nCheck Django terminal for details.");
//       }

//     } catch (err) {
//       console.error("❌ Network error — cannot reach Django:", err);
//       alert(
//         "❌ Cannot connect to Django server.\n\n" +
//         "Make sure it is running:\n\n" +
//         "  cd ~/service-provider-app/backend\n" +
//         "  source venv/bin/activate\n" +
//         "  python manage.py runserver 8002"
//       );
//     } finally {
//       bookingInProgress.current = false; // always release lock
//     }
//   };

//   const shownProviders = chosenIds.length === 0
//     ? allProviders
...
//             setSelectAllModal(allProviders.filter(p => ids.includes(p.id)));
//           }}
...
//             selectedProviders={selProviders} service={state?.service}
//             onSendAll={(desc) => { selProviders.forEach(pr => upd(pr.id, {description:desc, waSent:true})); setDescModalFor(null); }}
...
//             setPs(prev => { const u={...prev}; targets.forEach(pr => { u[pr.id]={...u[pr.id], description:desc, waSent:true}; }); return u; });
...
//       {/* ✅ ReviewModal — onBook={handleBookNow} is the ONLY DB saver */}
//       {reviewProvider && (
//         <ReviewModal
...
//                           <span style={{ color:"#F97316", fontWeight:700, fontSize:"9px", textTransform:"uppercase" }}>{provider.specialty}</span>
...
//                     <WorkflowBlock step={step} isYes={isYes} isNo={isNo} pst={pst} providerId={provider.id}
...
// ============================================================
// FILE: src/components/pages/user/ProviderList.js
// ✅ FIXED: handleBookNow sends ONE POST only (provider_names[])
//           No duplicate provider_id posts anywhere.
// ============================================================
...
export function ProviderList() {
  ...
  // ✅ Prevents duplicate API calls on double-click
  const bookingInProgress = useRef(false);
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // ✅ handleBookNow
  //    - Called ONLY from ReviewModal's "Book Now" button
  //    - Sends ONE POST to Django with provider_names[]
  //    - Django saves ONE DB row per provider name
  //    - Does NOT call addBooking with provider_id
  // ══════════════════════════════════════════════════════════════════════════
  const handleBookNow = async () => {
    if (bookingInProgress.current) return;   // block duplicates
    bookingInProgress.current = true;

    const p = reviewProvider;
    if (!p) { bookingInProgress.current = false; return; }

    // Collect ALL confirmed providers (not just the one open in modal)
    const confirmedProviders = shownProviders.filter(pr => ps[pr.id]?.confirmed);
    const providerNames = confirmedProviders
      .map(pr => pr.name?.trim())
      .filter(name => name && name.length > 0);

    // Fallback: use the provider currently open in the modal
    if (providerNames.length === 0) providerNames.push(p.name.trim());

    const service = state?.service || "General Service";
    const date    = new Date().toISOString().split("T")[0]; // YYYY-MM-DD

    console.log("🚀 Booking — sending to Django:", { provider_names: providerNames, service, date });

    try {
      // ✅ ONE POST — Django loops provider_names and saves one row per name
      const res = await fetch(`${API_BASE}/api/service-requests/`, {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ provider_names: providerNames, service, date }),
      });
      ...
      if (res.ok) {
        const savedCount = responseData.bookings?.length || providerNames.length;
        console.log(`✅ ${savedCount} booking(s) saved in database`);

        // Close review modal and mark provider reviewed
        handleCloseReview(p.id);
        ...
        // If not logged in — save pending and open login modal
        if (!currentUser) {
          sessionStorage.setItem("pendingProviderSelection", JSON.stringify(bookingData));
          if (window.__navbarOpenModal) window.__navbarOpenModal("login");
          bookingInProgress.current = false;
        ...
        // ✅ Save to localStorage via addBooking (no API call inside addBooking)
        addBooking({
          userId:    currentUser.id,
          userName:  currentUser.name,
          userEmail: currentUser.email,
        ...
        console.error("❌ Django error:", responseData);
        alert("❌ Booking failed: " + (responseData.error || "Unknown error"));
      }
  ...
  return (
    <div style={{ minHeight:"100vh", backgroundColor:"#F0F4F8", fontFamily:"'Segoe UI', system-ui, sans-serif" }}>
      ...
      {/* ✅ ReviewModal — onBook={handleBookNow} is the ONLY place that calls the Django API */}
      {reviewProvider && (
        <ReviewModal
          provider={reviewProvider}
          onClose={() => handleCloseReview(reviewProvider.id)}
